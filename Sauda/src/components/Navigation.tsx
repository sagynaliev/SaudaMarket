import { LayoutDashboard, ShoppingBag, Users, Store, Settings, Package, ShoppingCart, LogOut, Search, Bell, ChevronLeft, ChevronRight, User, MessageSquare, Heart, Clock } from 'lucide-react';
...
export function Sidebar({ role, activeTab, setActiveTab, collapsed, setCollapsed, onLogout }: SidebarProps) {
  ...
  const sellerLinks = [
    { id: 'dashboard', icon: LayoutDashboard, label: 'Sales Command' },
  ...
  const customerLinks = [
    ...
    { id: 'orders', icon: Clock, label: 'My Orders' },
    ...
    <header className="h-20 glass-card-dark rounded-none border-x-0 border-t-0 border-b-white/5 px-8 flex items-center justify-between sticky top-0 z-40">
      <div className="flex items-center gap-6 flex-1 max-w-xl">
        <div className="relative w-full group hidden md:block">
           <Search size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-accent transition-colors" />
           <input 
              type="text" 
              placeholder="Search assets, users, or protocols..."
              onChange={(e) => onSearch(e.target.value)}
              className="w-full h-11 bg-white/5 border border-white/5 rounded-xl pl-12 pr-4 text-sm focus:outline-none focus:border-accent/40 focus:bg-white/10 transition-all placeholder:text-slate-600"
           />
        </div>
      </div>
...
const HeartIcon = ({ size, className, fill }: { size: number, className?: string, fill?: string }) => <svg xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox="0 0 24 24" fill={fill || "none"} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z"/></svg>;
